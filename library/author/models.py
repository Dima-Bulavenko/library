...
class Author(models.Model):
    ...
    books = models.ManyToManyField(book.models.Book, related_name='authors', verbose_name='Книги')
    id = models.AutoField(primary_key=True)
    date_of_birth = models.DateTimeField(default=None, null=True, blank=True, verbose_name='Дата народження')
    date_of_death = models.DateTimeField(default=None, null=True, blank=True, verbose_name='Дата смерті')
    ...
    def create(name, surname, patronymic):
        ...
        if name not in [i.name for i in authors] or surname not in [i.surname for i in authors] or patronymic not in [i.patronymic for i in authors]:
